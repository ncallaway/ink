...
export async function identify(devicePath: DevicePath): Promise<Result<DiscId, Error>> {
  if (!devicePath) {
    return err(new Error(`A device path is required to identify a disc, but none was provided.`));
  }
  ...
  return err(new Error(`Could not generate and identifier for the disc in ${devicePath}`));
}
...
async function hashDvdStructure(videoTsPath: string): Promise<Result<DiscId, Error>> {
  try {
    ...
    return ok(hasher.digest('hex') as DiscId);
  } catch (e: any) {
    return err(e);
  }
}

async function hashBluRayStructure(bdmvPath: string): Promise<Result<DiscId, Error>> {
  try {
    const hasher = crypto.createHash('md5');
